  ...
  var QueryGenerator = {
    ...
    selectQuery: function(tableName, options) {
      ...
      options.attributes = options.attributes && options.attributes.map(function(attr) {
        var prefix = options.include ? options.table + '.' : ''
        ...
        if (Array.isArray(attr) && attr.length === 2) {
          ...
          return prefix + QueryGenerator.addQuotes(attr)
        }
      }).join(", ")
      ...
      if(options.where) {
        ...
        if (options.include) {
          var searchExp = /^(lower|upper|trim)\((\w+)\)/i

          if (searchExp.test(options.where)) {
            options.where = options.where.replace(searchExp, function(m, fnName, attributeName) {
              return fnName + '(' + options.table + '.' + attributeName + ')'
            })
          } else {
            options.where = options.table + '.' + options.where
          }
        }
